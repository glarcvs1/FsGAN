...
class DaGAN6_t(nn.Module):
    def __init__(self, config):
        super(DaGAN6_t, self).__init__()
        self.gen = Gen(config)
...
class unet_encoder(nn.Module):
    def __init__(self,ch):
        ...
        encode2 = [Conv2dBlock(ch * 2, ch * 4, 3, 2, 1,
                               norm='bn',
                               activation='lrelu',
                               pad_type='reflect')]
        self.encode2 = nn.Sequential(*encode2)
        # encode3 = [ActFirstResBlock(ch*2, ch*2)]
        encode3 = [Conv2dBlock(ch * 4, ch * 4, 3, 2, 1,
                               norm='bn',
                               activation='lrelu',
                               pad_type='reflect')]
        self.encode3 = nn.Sequential(*encode3)
        encode4 = [Conv2dBlock(ch * 4, ch * 4, 3, 1, 1,
                               norm='bn',
                               activation='lrelu',
                               pad_type='reflect')]
...
class unet_decoder(nn.Module):
    def __init__(self,ch,k):
        ...
        self.conv2 = nn.Conv2d(k['n_sample_train'] * ch * 4, ch * 4, (1, 1)) #62
        decode1 = [Conv2dBlock(ch * 4, ch * 4, 3, 1, 1,
                               norm='bn',
                               activation='lrelu',
                               pad_type='reflect')] #64
        decode1 += [UpSample(ch * 4, ch * 4)] #64
        self.decode1 = nn.Sequential(*decode1)
        self.att1 = attention(ch * 4, ch * 4)
        decode2 = [Conv2dBlock(ch * 8, ch * 2, 3, 1, 1,
                               norm='bn',
                               activation='lrelu',
                               pad_type='reflect')]
