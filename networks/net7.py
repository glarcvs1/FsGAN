...
class DaGAN7(nn.Module):
    def __init__(self, config):
        super(DaGAN7, self).__init__()
...
class unet_encoder(nn.Module):
    def __init__(self,ch):
        ...
        encode4 = [Conv2dBlock(ch * 2, ch * 2, 3, 2, 1,
                               norm='bn',
                               activation='lrelu',
                               pad_type='reflect')]
        self.encode4 = nn.Sequential(*encode4)
        encode5 = [Conv2dBlock(ch * 2, ch * 4, 3, 1, 1,
                               norm='bn',
                               activation='lrelu',
                               pad_type='reflect')]
        # encode4 += [nn.AvgPool2d(kernel_size=2, stride=2)]
        self.encode5 = nn.Sequential(*encode5)
    ...
    def forward(self,x):
        ...
        lay5 = self.encode5(lay4)
        encoder_layers.append(lay5)
        return lay5,encoder_layers

class unet_decoder(nn.Module):
    def __init__(self,ch):
        super(unet_decoder, self).__init__()
        self.concat = CropAndConcat()
        self.conv2 = nn.Conv2d(3 * ch * 4, ch * 2, (1, 1))
        ...
        self.att3 = attention(ch * 2, ch * 2)
        decode4 = [Conv2dBlock(ch * 4, ch * 2, 3, 1, 1,
                               norm='bn',
                               activation='lrelu',
                               pad_type='reflect')]
        decode4 += [UpSample(ch * 2, ch * 2)]
        self.decode4 = nn.Sequential(*decode4)
    ...
    def forward(self,x,encoder_layers,similarity,b,k):
        ...
        b1, c1, w1, h1 = encoder_layers[-5].size()
        encoder_layerss = encoder_layers[-5].view(b, k, c1, w1, h1)
        current_encoder = torch.multiply(sim, encoder_layerss)
        encoder_s_attention = self.att3(current_encoder, output)
        encoder_s_attention = torch.stack(encoder_s_attention, dim=1)
        encoder_s_attention = torch.sum(encoder_s_attention, dim=1)
        output = self.concat(output, encoder_s_attention)
        output = self.decode4(output)
...
if __name__ == '__main__':
    ...
    b=32
    k=3
    x = torch.randn(b*b,3, 128, 128).cuda()
    gen=unet_encoder(32).cuda()
    fake_x, similarity=gen(x)
    print(fake_x.size())
