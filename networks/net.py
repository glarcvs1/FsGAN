...
class Gen(nn.Module):
    def __init__(self,conflig):
        super(Gen, self).__init__()
        self.encoder = unet_encoder(32)
        self.decoder = unet_decoder(32)
        self.fusion = LocalFusionModule(inplanes=128, rate=conflig['rate'])
...
class unet_encoder(nn.Module):
    def __init__(self,ch):
        ...
        self.conv1 = nn.Conv2d(3, ch, (1,1))
        encode1=[ActFirstResBlock(ch,ch*2)]
        encode1+=[nn.AvgPool2d(kernel_size=2, stride=2)]
        encode1+=[nn.BatchNorm2d(ch*2)]
        self.encode1=nn.Sequential(*encode1)
        encode2 = [ActFirstResBlock(ch*2, ch*2)]
        encode2 += [nn.AvgPool2d(kernel_size=2, stride=2)]
        encode2 += [nn.BatchNorm2d(ch*2)]
        self.encode2 = nn.Sequential(*encode2)
        encode3 = [ActFirstResBlock(ch*2, ch*2)]
        encode3 += [nn.AvgPool2d(kernel_size=2, stride=2)]
        encode3 += [nn.BatchNorm2d(ch*2)]
        self.encode3 = nn.Sequential(*encode3)
        encode4 = [ActFirstResBlock(ch*2, ch*4)]
        #encode4 += [nn.AvgPool2d(kernel_size=2, stride=2)]
        encode4 += [nn.BatchNorm2d(ch*4)]
