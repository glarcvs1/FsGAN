...
def local_recon_criterion(real_xs: object, fake_x: object, similarity: object, indice_base: object, indice_refs: object, index: object, s: object = 8) -> object:
    """
    Local Reconstruction Loss 局部重建损失
    :param real_xs: real images (32*3*3*128*128)
    :param fake_x: generated fake images (32*3*128*128)
    :param similarity: alpha (32*3)
    :param indice_base: the recorded positions of selected base local representations (32*M)所选基本局部表示的记录位置
    :param indice_refs: the recorded positions of the matched reference representations (32*2*M) 匹配的引用表示的记录位置
    :param index: the index of base feature (1) 基本特征的索引
    :param s: resize the feature map 调整特性映射的大小
    :return:
    """
    base = real_xs[:, index, :, :, :]  # (32*3*128*128) #取出真实条件图像
    refs = torch.cat([real_xs[:, :index, :, :, :], real_xs[:, (index + 1):, :, :, :]], dim=1)  # (32*2*3*128*128) #支撑图像
    base_similarity = similarity[:, index]  # (32*1)  #条件图像的相似度
    ref_similarities = torch.cat([similarity[:, :index], similarity[:, (index + 1):]], dim=1)  # (32*2)  #支撑图像的相似度

    base = F.interpolate(base, size=s)  # (32*3*8*8)数据采样操作 size是最后两个维度的尺寸

    b, n, c, h, w = refs.size()
    refs = refs.view(-1, c, h, w)  #64*3*128*128
    refs = F.interpolate(refs, size=s)  #64*3*8*8
    ...
    target = batched_scatter(base, dim=2, index=indice_base, src=patch_fused) #将patch_fused的数据依靠indice扩增c分配到,shape=
