...
class DaGAN61(nn.Module):
    def __init__(self, config):
        super(DaGAN61, self).__init__()
        self.gen = Gen(config)
    ...
    def forward(self, xs, y, mode):
        if mode == 'gen_update':
            fake_x, similarity, indices_feat, indices_ref,base_index, s ,k= self.gen(xs)

            loss_recon = local_recon_criterion(xs, fake_x, similarity, indices_feat, indices_ref, base_index,
                                               s=s)  # 局部重建损失
...
class Gen(nn.Module):
    ...
    def forward(self, xs):
        ...
        fake_x = self.decoder(feat_gen,encoder_layers, similarity,b,k)

        return fake_x, similarity, indices_feat, indices_ref,base_index,s,k
...
class unet_decoder(nn.Module):
    def __init__(self,ch,k):
        ...
        self.conv2 = nn.Conv2d( ch * 4, ch * 2, (1, 1))
