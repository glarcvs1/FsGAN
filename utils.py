...
def make_result_folders(output_directory, remove_first=True):
    ...
    checkpoint_directory = os.path.join(output_directory, 'chs')
...
def write_images(iterations, dirs, im_outs, format='jpeg'):
    B, K2, C, H, W = im_outs.size()
    file_name = os.path.join(dirs, '%08d' % (iterations + 1) + '.' + format)
    image_tensor = im_outs
    image_tensor = image_tensor.view(B*(K2), C, H, W)
    image_grid = vutils.make_grid(image_tensor.data, nrow=K2, padding=0, normalize=True)
    vutils.save_image(image_grid, file_name, nrow=1, format=format)
...
def get_loader(dataset, root, mode, n_sample, num_for_seen, batch_size, num_workers, shuffle, drop_last,
    ...
    assert dataset in ['dce','flower', 'vggface', 'animal']
...
def batched_index_select(input: object, dim: object, index: object) -> object:   #
    views = [input.shape[0]] + [1 if i != dim else -1 for i in range(1, len(input.shape))]
    expanse = list(input.shape)
    expanse[0] = -1
    expanse[dim] = -1
    index = index.view(views) #reside
    index = index.expand(expanse)
    return torch.gather(input, dim, index)


def batched_scatter(input, dim, index, src):  #以dim=2为例
    views = [input.shape[0]] + [1 if i != dim else -1 for i in range(1, len(input.shape))] #【input.shape[0]，1，-1】
    expanse = list(input.shape)
    expanse[0] = -1
    expanse[dim] = -1   #expanse=[-1,input.shape[2],-1]
    index = index.view(views)  #[
    index = index.expand(expanse)
    return torch.scatter(input, dim, index, src) #将src数组中的数据重新分配到output数组当中，ind
    # ex数组中表示了要把src数组中的数据分配到output数组中的位置，若未指定，则填充0.
