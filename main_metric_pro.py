...
def fid(real, fake, gpu):
    ...
    command = 'python -m pytorch_fid {} {} --num-workers 0 '.format(real, fake)
    os.system(command)
    return command
...
parser.add_argument('--fake_dir', type=str,default='test_for_fid')
parser.add_argument('--ckpt', type=str, default='gen_00100006.pt')
...
if __name__ == '__main__':
    ...
    for k in range(11):
        ...
        fake_dir = os.path.join(args.name, args.fake_dir+str(k))
        ...
        if os.path.exists(fake_dir):
            ...
            if 0<k<10:
                last_model_name = os.path.join(args.name, 'chs', 'gen_000'+str(int(10000*k))+'.pt')
                print(last_model_name)
            elif k==0:
                last_model_name = os.path.join(args.name, 'chs','gen_0000' + str(int(10000 * (k + 1) / 2)) + '.pt')
                print(last_model_name)
            elif k==10:
                last_model_name = os.path.join(args.name, 'chs', 'gen_00' + str(int(10000*k)) + '.pt')
                print(last_model_name)
