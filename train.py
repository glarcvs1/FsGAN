...
from utils import make_result_folders, write_image, write_loss, get_config, get_loaders,write_images

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--conf', type=str, default='configs/vggface_lofgan.yaml')
    parser.add_argument('--output_dir', type=str, default='results/vggface_lofgan')
    ...
    checkpoint_directory, image_directory, log_directory = make_result_folders(output_directory,
            ...
            for it, (imgs, label) in enumerate(train_dataloader):
                ...
                if (iterations + 1) % config['snapshot_val_iter'] == 0:
                    with torch.no_grad():
                        ...
                        print('Saved image %d' % (iterations + 1))
