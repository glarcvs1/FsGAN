...
def main(args):
    ...
    traindata = npy.getdata(args.dir, args.mode, args.tb,args.te,args.leib,args.leie,args.prodir,nums, transform=traintransforms)
    ...
    scheduler = lr_scheduler.StepLR(optimizer, step_size=(args.s_size*kk), gamma=args.gam)
    since = time.time()
    keep_acc = 0.0
    keep_loss = 10
    #ter_cor=10
    ...
    for epoch in range(args.num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if (phase == 'val'):
                val_loss = epoch_loss
                val_acc=epoch_acc
            print('nums:{} {} Loss: {:.4f} Acc: {:.4f} keep_acc {:.4f}'.format(nums,phase, epoch_loss, epoch_acc,keep_acc),end='   ')
            # deep copy the model
            if phase == 'val' and val_acc > keep_acc:
                best_model_wts = copy.deepcopy(model.state_dict())
                keep_acc=val_acc
                if epoch>60:
                    strs = './output/' + args.msave + str(epoch) + '.pkl'
...
if __name__ == "__main__":
    for nums in [2,50,100,200, 400, 800,1600]:
        ...
        parser.add_argument('--s_size', type=int, default=6,help='多少步更新')
        #parser.add_argument('--num_classes', type=int, default=5) #类别数
        parser.add_argument('--mode', type=int, default=0) #训练集还是验证
        parser.add_argument('--num_epochs', type=int, default=num_epochs)
        parser.add_argument('--gam', type=float, default=0.65)
       # parser.add_argument('--dced', type=list, default=['AATH','DP','BRIX','ETOFTS','TOFTS'])
        parser.add_argument('--batch_size', type=int, default=256, help='批次的大小')
        parser.add_argument('--p', type=float, default=0.01, help='类别的比例')
        parser.add_argument('--lr', type=float, default=0.01)
        parser.add_argument('--lrf', type=float, default=0.1)
        parser.add_argument('--dex', type=int, default=0)
        parser.add_argument('--tb', type=int, default=30,help='ani=30,fllow=12,face=30') #验证集开始
        parser.add_argument('--te', type=int, default=65,help='ani=65,fllow=26,face=65')  # 验证集结束，测试集开始
        parser.add_argument('--leib', type=int, default=1802,help='ani=119,fllow=85,face=1802')  # 类别开始
        parser.add_argument('--leie', type=int, default=2355,help='ani=150,fllow=103,face=2355')  # 类别结束
        parser.add_argument('--dir', type=str,
                            default="input/vgg_face_data_rgb.npy",help='数据集的地址')
        parser.add_argument('--msave', type=str,
                            default="vggface10_1_"+str(nums)+"_pro6_res18_", help='模型保存名称')
        parser.add_argument('--prodir', type=str,
                            default="input/vggface_pro2000_6.npy", help='扩增的地址')
