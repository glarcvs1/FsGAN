...
class getdata(Dataset):
    def __init__(self,root_dir,mode,tb,tn,leib,leie,prove_dir=None,transform=None):
        #tb表示验证数据开始位置
        #tn表示测试数据开始位置
...
if __name__ == "__main__":
    ...
    prove_dir="input\\flower_c8189_s128_data_rgb.npy"
    data1 = np.load(prove_dir)
    #data = np.hstack((data[1:3, :1, :, :, :], data[1:3 ,3:, :, :, :]))
    print(data1.shape)
    data=getdata(dir,1,10,20,1,10)
