...
def main(args):
    ...
    traindata = npy.getdata(args.dir, args.mode, args.tb, args.te, args.leib, args.leie, args.prodir,args.pronums,transform=traintransforms)
    ...
    for epoch in range(args.num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if (phase == 'val'):
                val_loss = epoch_loss
                val_acc=epoch_acc
            print('nums:{} {} Loss: {:.4f} Acc: {:.4f}'.format(li,phase, epoch_loss, epoch_acc),end='   ')
            # deep copy the model
...
if __name__ == "__main__":
    for li in range(5):
        ...
        parser.add_argument('--leib', type=int, default=16+(2*li), help='ani=119,fllow=85,dce=16,18,20,22,24')  # 类别开始
        parser.add_argument('--leie', type=int, default=18+(2*li), help='ani=150,fllow=103,dce=18,20,22,24,26')  # 类别结束
        parser.add_argument('--dir', type=str,
                            default="input/dce125.npy", help='数据集的地址')
        parser.add_argument('--msave', type=str,
                            default="dce_res18_"+dced[li], help='模型保存名称')
        parser.add_argument('--prodir', type=str,
                            default="input/dce_pro2000_6.npy", help='数据集的地址')
        parser.add_argument('--pronums', type=int, default=100,help="扩增数量")
